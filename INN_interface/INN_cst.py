...
class INN():
    ...
    def generate_polars(self, cst, Re, alpha=np.arange(-4, 20.1, 0.25), af=None, KL_basis=True):
        ...
        for i in range(N_shapes):
            ...
            if self.models[af[i]].xM > x.shape[1]:
                ...
                x_append = np.repeat([[6.01666709e-07, 4.97110775e-07]], x.shape[0], axis=0)
    ...
    def inverse_design(self, cd, clcd, stall_margin, thickness, Re, 
        ...
        if (z is None):
            ...
            z_val = tf.random.normal([N, self.models[af].zM], dtype=tf.float64)
        ...
        x_inv, y_inv = self.models[af].eval_inverse(y_val, c_val, f_val, z_val)
        
        # Sort generated shapes by errors in network forward prediction
        if process_samples:
            x_inv, y_inv = self.sort_by_errs(x_inv, y_inv, y_val, c_val, f_val, af, N=N)
        ...
        y_inv = unnorm_data(y_inv, scale_factor=self.scale_factors[af]['y']).numpy()
    ...
    def sort_by_errs(self, x_inv, y_inv, y, c, f, af, N=1):
        # Sort shapes by total errors
        y_out, c_out, f_out, _ = self.models[af].eval_forward(x_inv, y)
        ...
        y_inv = unnorm_data(y_inv, scale_factor=self.scale_factors[af]['y'])
        ...
        x_inv, y_inv = x_inv.numpy(), y_inv.numpy()

        # Compute relative errors for each output
        y1_err = np.sum(np.abs((y - y_inv)/y), axis=1)
        y2_err = np.sum(np.abs((y_out - y_inv)/y), axis=1)
        ...
        x_inv = x_inv[idx[:N], :]
        y_inv = norm_data(y_inv[idx[:N], :], scale_factor=self.scale_factors[af]['y'])
